...
class TestTasks:
    ...
    async def test_get_task(self):
        async with AsyncClient(transport=ASGITransport(app), base_url="http://test") as client:
            ...
            assert response.status_code == 200 and response.json()[0]["title"] == "test1"

    async def test_add_task(self):
        async with AsyncClient(transport=ASGITransport(app), base_url="http://test") as client:
            task = {"title" : "test4", "description" : "test4", "deadline" : date(year=2025, month=5, day=19).isoformat(),
    ...
    async def test_complete_task(self):
        async with AsyncClient(transport=ASGITransport(app), base_url="http://test") as client:
            ...
            assert response.status_code == 200 and response.json()[0]["status"] == "done"